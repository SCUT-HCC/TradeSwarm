...
TradeSwarm是一个基于多智能体协作的量化交易系统，采用多线程并行Pipeline架构设计。系统通过专业化的信息处理Agent和研究分析Agent协同工作，最终由交易决策Agent生成交易指令。项目架构参考了TradingAgents项目，但在并发执行和信息流设计上进行了创新优化。
...
| 维度 | TradingAgents | TradeSwarm |
|------|---------------|------------|
| **架构模式** | 单线程顺序执行 | 多线程并行Pipeline |
| **信息处理** | Agent逐个处理信息 | Layer 1四个Agent并行处理 |
| **研究分析** | 单一分析流程 | Bullish/Bearish双向分析+迭代优化 |
| **并发控制** | 无 | AgentPool+RateLimiter双层限流 |
| **执行效率** | 较低（串行等待） | 高（并行+异步） |
| **可扩展性** | 有限 | 高（支持大规模Agent调度） |
